...
const Navbar = () => {
  const {homeRef, aboutRef, skillRef, projectsRef, exprenceRef, serviceRef, learningRef, educationRef, contactRef, scrollToSection, activeSection} = useContext(AllassetContet)
  ...
  return (
    ...
    <div className="py-4 relative top-0 z-10 flex justify-around items-center bg-gray-600/95 shadow">
      <div className=" w-10">
      ...
      <div className=" hidden sm:block ">
        <ul className=" flex gap-10">
          <li>
            <button onClick={()=>scrollToSection(homeRef, 'home')} className={`text-lg hover:text-green-700 active:text-amber-200 cursor-pointer  ${activeSection === 'home' ? 'text-green-600': ''}`}> Home </button>
          </li>
          <li>
            <button onClick={()=>scrollToSection(aboutRef, 'about')} className={`text-lg hover:text-green-700 active:text-amber-200 cursor-pointer  ${activeSection === 'about' ? 'text-green-600': ''}`}> About </button>
          </li>
          <li>
            <button onClick={()=>scrollToSection(skillRef, 'skill')} className={`text-lg active:text-amber-200 hover:text-green-700 cursor-pointer ${activeSection === 'skill' ? 'text-green-600' : ''}`}> Skill </button>
          </li>
          <li>
        <button onClick={()=>scrollToSection(projectsRef, 'project')} className={`text-lg active:text-amber-200 hover:text-green-700 cursor-pointer ${activeSection === 'project' ? 'text-green-600' : ''}`}> Project </button>
          </li>
          <li>
        <button onClick={()=>scrollToSection(exprenceRef, 'experince')} className={`text-lg active:text-amber-200 hover:text-green-700 cursor-pointer ${activeSection === 'experince' ? 'text-green-600' : ''}`}> Experince </button>
          </li>
          <li>
        <button onClick={()=>scrollToSection(learningRef, 'learning')} className={`text-lg active:text-amber-200 hover:text-green-700 cursor-pointer ${activeSection === 'learning' ? 'text-green-600' : ''}`}> Learing </button>
          </li>
          <li>
        <button onClick={()=>scrollToSection(contactRef, 'contact')} className={`text-lg active:text-amber-200 hover:text-green-700 cursor-pointer ${activeSection === 'contact' ? 'text-green-600' : ''}`}> Contact </button>
          </li>                            
        </ul>
      </div>
      <div className="hidden sm:block">
            <Link to='/webresumelayout' className={`text-lg active:text-amber-200 hover:text-green-700 ${isActive('/webresumelayout') ? 'text-green-600' : ''}`}> Resume </Link>        
      </div>
    ...
    <div onClick={ManubarHander} className={`sm:hidden flex flex-col bg-blue-400/95 absolute top-full w-full h-[92.3vh] justify-center items-center space-y-10 ${manubar ? 'block' : 'hidden'}`}>
      ...
      <button onClick={()=>scrollToSection(homeRef, 'home')} className={`text-lg text-black hover:text-green-700 active:text-amber-200 cursor-pointer  ${activeSection === 'home' ? 'text-green-600': ''}`}> Home </button>
